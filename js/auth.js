...
const AuthManager = {
    ...
    async requestPhoneOtp() {
        ...
        const userName = document.getElementById("phoneUserNameInput")?.value?.trim() || "";
        ...
        try {
            ...
            // Populate test demo hint
            if (data.demo_otp) {
                this.showAlert(`SMS Simulation: Verification PIN is <strong>${data.demo_otp}</strong>`, "info");
                // Pre-fill digits for effortless testing
                const boxes = document.querySelectorAll(".otp-box");
                for (let i = 0; i < boxes.length && i < data.demo_otp.length; i++) {
                    boxes[i].value = data.demo_otp[i];
                }
            }
        } catch (err) {
            this.showAlert(err.message, "error");
    ...
    async handlePhoneOtpSubmit() {
        ...
        const userName = this.state.userName || document.getElementById("phoneUserNameInput")?.value?.trim() || "";
